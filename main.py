...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    if is_best:
      if args.refine:
        shutil.copyfile(filename, 'model_best_refine.pth.tar')
      else:
...
for epoch in range(args.start_epoch, args.epochs):
    ...
    if(epoch == args.epochs -1):
        if(args.refine):
            file.write("Refined with accuracy: {}\n".format(prec1))
        else:
            file.write("Trained Model with sparsity {} and accuracy {}\n".format(args.s, prec1))
file.close()

#
# pruning = [0.05, 0.1, 0.2, 0.35, 0.5, 0.65, 0.8]
# sparsity = [.00001, .001, 1]
# file = open("results.txt", "a")
# file.close()
#
...
#     file.close()
#     train = subprocess.Popen(['python', 'main.py', '-sr', '--epochs', '3', '--s', str(arg1)])
#     train.wait()
#      remove checkpoint!!!!!
...
#             file.close()
#             continue
#         train = subprocess.Popen(['python', 'main.py', '--refine', 'pruned4.pth.tar', '--epochs', str(3)])
#         train.wait()
